...
export const createCamera = () => {
    const camera = new THREE.PerspectiveCamera(3, document.getElementById("modelPlace").clientWidth / document.getElementById("modelPlace").clientHeight, 20, 4000);
...
export const createModel = (model, scene, path, setLoading,renderer) => { //model is a react ref, remember that
    ...
    loader.load(path, function (gltf) {
        ...
        bagLayer.children.map((item)=>{
            if(item.isMesh){
            //   item.material.color=new THREE.Color('#B3AEB0')
